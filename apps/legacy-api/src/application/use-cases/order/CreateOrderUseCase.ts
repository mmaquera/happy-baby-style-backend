    ...
    try {
      ...
      const productValidations = await Promise.all(
        orderData.items.map(async (item) => {
          ...
          if (!variant) {
            throw new Error(
              `Variant with size ${item.size} and color ${item.color} not available for product ${product.name}`,
            );
          }

          if (variant.stockQuantity < item.quantity) {
            throw new Error(
              `Insufficient stock for variant ${item.size}/${item.color} of product ${product.name}. Available: ${variant.stockQuantity}, Requested: ${item.quantity}`,
            );
          }
