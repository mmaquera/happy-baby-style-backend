...
export class PrismaImageRepository implements IImageRepository {
  ...
  async create(image: ImageEntity): Promise<ImageEntity> {
    try {
      ...
      this.logger.error('Error creating image', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async findAll(filters?: ImageFilters): Promise<ImageEntity[]> {
    try {
      ...
      this.logger.error('Error finding images', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async findByEntityId(entityId: string, entityType: ImageEntityType): Promise<ImageEntity[]> {
    try {
      ...
      this.logger.error('Error finding images by entity', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async deleteByEntityId(entityId: string, entityType: ImageEntityType): Promise<void> {
    try {
      ...
      this.logger.error('Error deleting images by entity', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
